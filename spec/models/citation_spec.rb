...
RSpec.describe Citation, type: :model do
  describe "validations" do
    ...
    it { is_expected.to_not validate_uniqueness_of(:book).case_insensitive.scoped_to(%i[chapter verse]) }

    it { is_expected.to validate_presence_of(:chapter) }
    it { is_expected.to_not validate_uniqueness_of(:chapter).scoped_to(%i[book verse]) }
    it { is_expected.to validate_numericality_of(:chapter).only_integer }

    it { is_expected.to validate_presence_of(:verse) }
    it { is_expected.to_not validate_uniqueness_of(:verse).scoped_to(%i[book chapter]) }
